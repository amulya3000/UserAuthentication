...
<body class="bg-slate-50 text-slate-800 flex min-h-screen relative overflow-x-hidden font-sans">
    ...
    <div class="fixed top-[-20%] left-[-10%] w-[50vw] h-[50vw] rounded-full bg-emerald-300/20 blur-[120px] pointer-events-none"></div>
    <div class="fixed bottom-[-20%] right-[-10%] w-[50vw] h-[50vw] rounded-full bg-emerald-300/20 blur-[120px] pointer-events-none"></div>

    <!-- Sidebar -->
    <aside class="w-64 glass border-r border-slate-200/50 p-5 hidden md:flex flex-col gap-8 z-10 sticky top-0 h-screen">
        <a href="{{ route('dashboard') }}" class="flex items-center justify-center gap-2 px-4 py-2.5 bg-emerald-950 text-white rounded-xl text-sm font-semibold shadow-lg shadow-slate-900/20 hover:bg-slate-800 transition-all hover:-translate-y-0.5">
        ...
        <div>
            ...
            <nav class="flex flex-col gap-1.5">
                ...
                <a href="{{ route('sprint') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium bg-emerald-50/80 text-emerald-700 shadow-sm border border-emerald-100/50">
    ...
    <main class="flex-1 p-8 md:p-12 z-10">
        <div class="max-w-5xl mx-auto">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 gap-4">
                <div>
                    <p class="text-xs text-emerald-600/80 font-semibold tracking-wide uppercase mb-1">Projects / To-Do App</p>
                ...
                <form action="{{ route('sprints.store') }}" method="POST" class="flex items-center gap-2 bg-white/50 p-1.5 rounded-xl border border-slate-200/60 shadow-sm backdrop-blur-md">
                    ...
                    <button type="submit" class="bg-emerald-600 hover:bg-emerald-700 text-white font-medium px-4 py-2 rounded-lg text-sm transition-all shadow-md shadow-emerald-600/20 flex items-center gap-2 hover:-translate-y-0.5">
            ...
            <div class="space-y-8">
                ...
                <div class="glass rounded-2xl border {{ $sprint->is_active ? 'border-emerald-400/50 shadow-xl shadow-emerald-900/5' : 'border-slate-200/60 shadow-lg shadow-slate-200/40' }} p-6 md:p-8 transition-all hover:shadow-xl">
                    <form action="{{ route('sprints.update', $sprint->id) }}" method="POST" class="mb-6">
                        ...
                        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                            <div class="flex items-center gap-4 flex-wrap">
                                ...
                                @elseif($sprint->is_active)
                                    <span class="bg-emerald-600 text-white text-[11px] px-3 py-1 rounded-full font-bold tracking-wide uppercase shadow-sm shadow-emerald-600/30 flex items-center gap-1.5">
                        ...
                        <div class="mb-4">
                            <textarea name="notes" placeholder="Add sprint goals, outcomes, or notes here..." class="w-full text-sm px-4 py-3 border border-slate-200/80 rounded-xl bg-white/50 focus:bg-white focus:ring-2 focus:ring-emerald-100 focus:border-emerald-300 transition-all shadow-sm placeholder:text-slate-400" rows="2">{{ $sprint->notes }}</textarea>
                        </div>

                        <div class="flex justify-end">
                            <button type="submit" class="text-xs bg-slate-800 hover:bg-emerald-950 text-white px-4 py-2 rounded-lg font-medium transition-all shadow-md shadow-slate-800/10 hover:-translate-y-0.5">
                                Save Sprint Details
                            </button>
                        </div>
                    </form>
                    ...
                    <div class="bg-white/60 rounded-xl p-5 border border-slate-100 shadow-sm mb-6">
                        <div class="flex justify-between items-end mb-2">
                            ...
                            <span class="text-xl font-black {{ $pct == 100 ? 'text-green-600' : 'text-emerald-600' }}">{{ $pct }}%</span>
                        </div>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-5 gap-3 mt-4">
                            ...
                            <div class="bg-white border border-slate-100 rounded-lg p-3 text-center shadow-sm">
                                <span class="block text-2xl font-bold text-emerald-600">{{ $counts['in_progress'] }}</span>
                                ...
                                <div class="flex items-center justify-between p-3 hover:bg-slate-50/80 transition-colors group">
                                    ...
                                    <div class="flex items-center gap-3">
                                        <span class="text-[11px] px-2.5 py-1 rounded-md font-semibold border 
                                            {{ $item->type == 'Story' ? 'bg-emerald-50 border-emerald-100 text-emerald-600' : '' }}
                                            {{ $item->type == 'Task' ? 'bg-emerald-50 border-emerald-100 text-emerald-600' : '' }}
                                        ...
                                        <span class="text-[11px] px-2.5 py-1 rounded-md font-semibold border
                                            ...
                                            {{ $item->status == 'In Progress' ? 'bg-emerald-50 border-emerald-100 text-emerald-600' : '' }}
                    ...
                    <form action="{{ route('sprint.issue.store', $sprint->id) }}" method="POST" class="flex gap-3 bg-slate-50/80 p-2 rounded-xl border border-slate-200/60">
                        ...
                        <input 
                            ...
                            class="flex-1 px-4 py-2 border-none bg-white rounded-lg focus:ring-2 focus:ring-emerald-100 shadow-sm text-sm"
                        />
                        <select name="type" class="px-3 py-2 border-none bg-white rounded-lg text-sm shadow-sm focus:ring-2 focus:ring-emerald-100 font-medium text-slate-600 cursor-pointer">
                            <option value="Task">Task 📝</option>
                            <option value="Story">Story 📚</option>
                            <option value="Bug">Bug 🐛</option>
                        </select>
                        <button type="submit" class="bg-slate-800 hover:bg-emerald-950 text-white font-medium px-5 py-2 rounded-lg text-sm transition-all shadow-md hover:-translate-y-0.5">
                            Add Issue
                        </button>
                    </form>
                    @endif
